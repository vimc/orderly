...
test_that("empty", {
  ...
  expect_equal(orderly_drafts(path),
...
test_that("query through lifecycle", {
  ...
  empty <- data.frame(name = character(0), id = character(0),
                      stringsAsFactors = FALSE)
  expect_equal(orderly_drafts(path), empty)
  expect_equal(orderly_archive(path), empty)

  p <- orderly_run("example", config = path, echo = FALSE)

  r <- orderly_drafts(path)
  ...
  expect_equal(orderly_drafts(path), empty)
  expect_equal(orderly_archive(path), r)
})
