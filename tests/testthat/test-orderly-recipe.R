...
test_that("dependencies must be scalar", {
  ...
  expect_error(orderly_recipe$new("depend", orderly_config_$new(path)),
...
test_that("can't use connection in configurations that lack databases", {
  ...
  writeLines(c(txt, yaml::as.yaml(dat)), p)
  expect_error(
    suppressWarnings(orderly_recipe$new("example", orderly_config_$new(path))),
...
test_that("old style global resources deprecated", {
  ...
  expect_warning(
    res <- orderly_recipe$new("example", orderly_config_$new(path)),
...
test_that("Can resolve dependencies remotely", {
  ...
  config <- orderly_config_$new(dat$path_local)
