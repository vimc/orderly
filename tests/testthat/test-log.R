...
test_that("orderly can produce progress messages", {
  progress <- tryCatch(orderly_progress(list(id = "123")),
                       condition = identity)
  expect_s3_class(progress, "progress")
  expect_s3_class(progress, "condition")
  expect_equal(progress$id, "123")
})
