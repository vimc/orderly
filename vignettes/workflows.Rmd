...
* who's outputs are the "canonical" version?
...
* if the report is stochastic, the results will likely differ
...
What we want is that after a particular version of a report to have been run, it is easy for other people to start depending on it.  To achive this we need a method for getting the archive from one person's computer to another.
...
Alice develops a report `report-a` on her computer by running `orderly::orderly_new("report-a")` and working with `orderly::orderly_develop_start()` and `orderly::orderly_run()` until she is happy with the results.  She then creates a pull request, and Bob approves it (probably after checking that the report runs on his machine too) and merged it.
...
```yaml
ORDERLY_PATH_DEFAULT_REMOTE: /Users/alice/Dropbox/ourorderly
```

and for Bob it might look like

```yaml
ORDERLY_PATH_DEFAULT_REMOTE: c:/Bob/Documents/Dropbox/ourorderly
```
...
If you are working on a report that has a dependency (say `report-b`, which depends on `report-b`), you can automatically pull its dependencies by running
...
* Because every report gets a unique identifier, it is straightforward to combine outputs that were run on different machines.  In the example above this is a centralised system (one canonical source, several destinations) but in theory more complicated distributted workflows are possible.
...
If OrderlyWeb has beeen deployed onto `orderly.example.com`, then we might declare it in `orderly_config.yml`
...
this will print to screen the log, just as if it was running locally, though it is in fact running on a fresh R session remotely.  After being run, this report is available to everyone and can be pulled as above
...
Now, as part of the pull request review proceedure, the reviewer will run the report on the staging environment, which ensures that it will likely work without error on the production environment.
...
Now, even if she has more recent copies of the dependncies locally, her report will use the same copies as would be used on the server she choses.
...
To support this, we can have a network of orderly servers which can exchange subtrees amongst each other.  In this fashion, we can imagine the following things being possible
