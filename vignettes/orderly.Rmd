...
Resources to a report are expected to be static files that are used by 
the script to produce the report. Examples of the sort files that
...
Sources are files containing R code that sourced (via the R function
`source()`) before the main script is run. Often this file contains
functions or variables used by the main script.

Artefacts are the output to the report. At least one artefact must be
