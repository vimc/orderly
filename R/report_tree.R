report_tree <- R6::R6Class(
  ...
  private = list(
    ...
    format_helper = function(vertex, prefix, tree_string, chars) {
      ...
      children <- private$get_children(vertex)
    ...
    vertex_to_dep = function(vertex) {
      children <- private$get_children(vertex)
        list(
          ...
          dependencies = lapply(children, private$vertex_to_dep)
        )
    }
  ),
